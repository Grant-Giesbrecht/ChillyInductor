...
parser.add_argument("--V0", type=float, default=0.05, help="Stimulus amplitude [V]")
parser.add_argument("--Vmin", type=float, default=0, help="Min total length [m]")
parser.add_argument("--Vmax", type=float, default=1, help="Max total length [m]")
parser.add_argument("--num_sweep", type=int, default=15, help="Number of sweep points")
...
parser.add_argument("--T", type=float, default=30e-9, help="Total simulation time [s]")
...
def process_ouput(out, pl, hp_obj):
	...
	print(f"Phy. Length={pl:.3f} m  ->  P1={P1_dBm:.2f} dBm,  P2={P2_dBm:.2f} dBm,  P3={P3_dBm:.2f} dBm")
...
def main():
	...
	length_vals = np.linspace(0, 1, 31)
	length_vals = length_vals[1:]
	...
	if args.parallel:
		...
		# Unpack results
		for pl, res in zip(length_vals, results):
		...
		for pl in length_vals:
			
			# Define system parameters
			fdtd_params_exp, le_params_exp = define_system(pl, args.f0, args.V0, args.T, args.dx_ref, False, V_bias=V_bias) # Explicit params
			# fdtd_params_imp, le_params_imp = define_system(pl, args.f0, args.V0, args.T, args.dx_ref, True, V_bias=V_bias) # Implicit params
			
			fdtd_exp_sim = FiniteDiffSim(fdtd_params_exp) # Create sim
			fdtd_exp_out = fdtd_exp_sim.run() # run
			process_ouput(fdtd_exp_out, pl, fdtd_exp_powers)
...
# Convert list of results into 'transposed' objects
	...
	# Modeling series resistance as 1kOhm, which is a close approximation. 
	plt.plot(length_vals, fdtd_exp_powers.P1_list, marker='x', label='Fundamental, Explicit', color=c_fund, linestyle='--')
	plt.plot(length_vals, fdtd_exp_powers.P2_list, marker='x', label='2nd harmonic, Explicit', color=c_2h, linestyle='--')
	plt.plot(length_vals, fdtd_exp_powers.P3_list, marker='x', label='3rd harmonic, Explicit', color=c_3h, linestyle='--')
	
	# plt.plot(length_vals, fdtd_imp_powers.P1_list, marker='+', label='Fundamental, Implicit', color=c_fund, linestyle=':')
	# plt.plot(length_vals, fdtd_imp_powers.P2_list, marker='+', label='2nd harmonic, Implicit', color=c_2h, linestyle=':')
	# plt.plot(length_vals, fdtd_imp_powers.P3_list, marker='+', label='3rd harmonic, Implicit', color=c_3h, linestyle=':')
	...
	dataset = {}
	dataset['phys_lengths'] = list(length_vals)
	dataset['P1'] = list(fdtd_exp_powers.P1_list)
	dataset['P2'] = list(fdtd_exp_powers.P2_list)
	dataset['P3'] = list(fdtd_exp_powers.P3_list)
	
	with open("sp3f_results.json", 'w') as fh:
