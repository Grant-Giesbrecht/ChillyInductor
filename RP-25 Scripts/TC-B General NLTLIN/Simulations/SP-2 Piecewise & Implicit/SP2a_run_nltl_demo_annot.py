...
from nltl_sim import (
    demo_fdtd_piecewise_v2, demo_ladder_piecewise_v2,
    FDTDRegion, FDTDParamsPW, LadderRegion, LadderParamsPW
)

# -------------------------
# Run simulations (once)
# -------------------------
fdtd_out, fdtd_p = demo_fdtd_piecewise_v2()
ladder_out, ladder_p = demo_ladder_piecewise_v2()
...
for (x0, x1, Z0, alpha) in fdtd_labels:
    label_lines.append(f"[{x0:.3f}–{x1:.3f} m]  Z0≈{Z0:.1f} Ω,  α={alpha:.2e}")
label_text = "\n".join(label_lines)

# Put the annotation inside the top axes, upper-right corner
annot = ax_top.text(
    0.98, 0.05, label_text, transform=ax_top.transAxes,
    ha='right', va='bottom', fontsize=9,
    bbox=dict(boxstyle='round', alpha=0.2, lw=0.0)  # default facecolor
)

# Time stamp
time_txt = ax_top.text(0.02, 0.93, f"t = {t_common[0]:.3e} s",
                       ha='left', va='center', transform=ax_top.transAxes)
