...
"""
run_nltl_demo.py (synchronized two-panel animation; robust no-blit)
-------------------------------------------------------------------
Top:  FDTD V(x, t)
Bottom: Ladder V(node->x, t)

Notes:
- blit=False avoids backend-specific AttributeError: 'NoneType' object has no attribute '_get_view'
- timestamp text is drawn inside ax_top to avoid figure-level blit glitches
"""
...
time_txt = ax_top.text(0.98, 0.92, f"t = {t_common[0]:.3e} s",
                       ha='right', va='center', transform=ax_top.transAxes)
