...
%
	...
	expectedDomain = {'SYSTEM', 'CHIP', 'SYSTEMS11'};
	% System = P2VNA/PgenSet
	% Chip = P2VNA/(P1VNA + P2VNA + P3VNA)
	% System S11 = P2VNA/(PgenSet * (1-S11linear))
	
	sparams_null = sparameters([1,0;0,1], 1, 50);
	...
	p.addParameter('CEDefinition', false, @(x) any(validatestring(upper(char(x)),expectedDomain)) );
	p.addParameter('S2PData', sparams_null, @(x) isa(x, 'sparameters') );
	...
	%% Extract maximum CE over freq, with a trace for each power
		...
		has_S2P_data = false;
		if p.S2PData.Frequencies ~= sparams_null.Frequencies
			has_S2P_data = true;
		elseif p.S2PData.Parameters ~= sparams_null.Parameters
			has_S2P_data = true;
		elseif p.S2PData.Impedance ~= sparams_null.Impedance
			has_S2P_data = true;
		end
		...
		% 	multiWaitbar('Generate Figure 4', (pidx-1)/numel(powers_dBm));
			...
			for f = freqs
				...
				% Find S11 at this frequency
				if has_S2P_data
					s11q = interp1(p.S2PData.Frequencies, abs(flatten(p.S2PData.Parameters(1, 1, :))), f);
					
					% If interp fails, means data out of range. Mark as no
					% data available.
					if isnan(s11q)
						warning("Failed to use provided S2P data. Frequencies out of range.");
						has_S2P_data = false;
					end
					s11q_all(idx) = s11q;
				end
				...
				if has_S2P_data
					...
					CE3syss11_ = abs(harm_struct.h3)./(cvrt(pwr, 'dBm', 'W')*(1-s11q)^2).*100;
					[CE2_syss11(idx), mi2s] = max(CE2syss11_);
					[CE3_syss11(idx), mi3s] = max(CE3syss11_);
				end
			end
			...
			if strcmpi(p.CEDefinition, 'SYSTEM')
				...
				CE3_sel = CE3_sys;
				ce_type_str = "System";
			elseif strcmpi(p.CEDefinition, 'CHIP')
				CE2_sel = CE2;
				CE3_sel = CE3;
				ce_type_str = "Chip";
			elseif strcmpi(p.CEDefinition, 'SYSTEMS11')
				
				if ~has_S2P_data
					error("Cannot calculate SystemS11 CE without S2P Data");
				end
				
				CE2_sel = CE2_syss11;
				CE3_sel = CE3_syss11;
				ce_type_str = "System+S11";
			end
