...
% Previously names: est_I4.m
%
% This script tries to fit an equation to phase change data (like SC1), but
% this script uses and I^4 model (with new maths). 
...
% Load frequencies
load(fullfile("script_assets", "SC1", "keep_freqs_full.mat"));
	...
	if reselect_frequencies
		...
		while true
			...
			elseif keep == 'exit'
				...
				save(fullfile("script_assets", "SC1", 'keep_freqs.mat'), 'keep_freqs', 'trash_freqs', 'border_freqs');
				return
			end
		end		
	end	
