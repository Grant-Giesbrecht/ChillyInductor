...
# Time to wait in between temp checks during equilibration procedure
EQUILIB_PERIOD_S = 10
...
try:
	...
	temp_list_K = interpret_range(conf_data['temp_list_K'], print_err=True)
...
cs_time = str(datetime.datetime.now())
temp_meas = tempctrl.get_temp()
log.debug(f"Measured temperature at end of current-set resistance calibration: {temp_meas} K", detail=f"Timestamp = {cs_time}")

abort = False
holdoff_count = 0

# Scan over all temperatures
for temp_set in temp_list_K:
	
	# Change setpoint
	log.info(f"Setting target temperature to >{temp_set}< K.")
	tempctrl.set_setpoint(temp_set)
	tempctrl.set_range(LakeShoreModel335.RANGE_MID)
	tempctrl.set_enable(True)
	
	log.debug(f"Beginning equilibration procedure.")
	equilibrating = True
	while equilibrating:
		
		time.sleep(EQUILIB_PERIOD_S)
		
		# Measure current temperature
		temp_meas = tempctrl.get_temp()
		log.debug(f"Equilibrating... measured temperature {temp_meas} K")
		
		# Record value
		dataset['aux_dataset']['continuous_temp_logging']['temp_setpoint_K'].append(temp_set)
		dataset['aux_dataset']['continuous_temp_logging']['temp_K'].append(temp_meas)
		dataset['aux_dataset']['continuous_temp_logging']['timestamp'].append(str(datetime.datetime.now()))
		
		# Check if the system has equilibrated
		if len(dataset['aux_dataset']['continuous_temp_logging']['temp_K']) > 21:
			...
			if holdoff_count > 0:
				...
				old_temp = np.mean(dataset['aux_dataset']['continuous_temp_logging']['temp_K'][-19:-15])
				new_temp = np.mean(dataset['aux_dataset']['continuous_temp_logging']['temp_K'][-4:-1])
				temp_delta = np.abs(new_temp - old_temp)
				temp_targ_delta = np.abs(new_temp - temp_set)
				
				# Check for sufficient change
				if (temp_delta < DELTA_THRESHOLD_T) and (temp_targ_delta < DELTA_THRESHOLD_TARG):
					log.info(f"Achieved equilibrium condition. Exiting equilibration routine.", detail=f"delta T ({temp_delta} K) < threshold ({DELTA_THRESHOLD_T} K), target deviation ({temp_targ_delta} K) < threshold ({DELTA_THRESHOLD_TARG} K). Old average = {old_temp} K, new average = {new_temp} K.")
					
					# Instruct to exit equilibration phase
					equilibrating = False
					
					
					# Instruct to wait for 20 measurement cycles before checking for equilibrium again
					holdoff_count = 20
...
if abort:
	...
	log.info("Sweep completed. Shutting off signal generator, heater and DC bias.")

try:
	tempctrl.set_setpoint(1)
	tempctrl.set_enable(False)
	tempctrl.set_range(LakeShoreModel335.RANGE_OFF)
except Exception as e:
	log.warning(f"Failed to turn off heater.", detail=f"{e}")
