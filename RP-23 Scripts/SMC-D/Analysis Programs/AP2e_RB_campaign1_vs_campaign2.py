...
##-------------------------------------------------------
# Data copied over from RP23/SMC-A/*/AP3a_sigma_vs_error_rate_v3_pub.py

RP23A_trad_sigma = [10.0, 14.999999999999998, 20.0, 25.0, 29.999999999999996, 40.0, 50.0, 100.0, 200.0]
RP23A_trad_err = [0.00631, 0.0074, 0.0082, 0.009, 0.00938, 0.012, 0.0132, 0.027, 0.0492]

RP23A_doubler_err = [0.0151, 0.0111, 0.00994, 0.0116, 0.00935, 0.0116, 0.0127, 0.0245, 0.0559]
...
tau = 1.875*expected_err_x_us*8
expected_err_y_F = 1/6*(3+np.exp(-tau/t1_)+2*np.exp(-tau/t2_))
y1 = np.array(1-expected_err_y_F)
...
ax2a.errorbar(trad_sigmas, np.array(trad_err_uncs)*1e3, yerr=np.array(trad_std_uncs)*1e3, linestyle='--', marker=marker_trad[0], color=COLOR_TRAD, label="Direct drive, new measurement system", capsize=CSIZE, alpha=ERROR_ALPHA, markersize=marker_trad[1])
ax2a.errorbar(doubler_sigmas[1:], np.array(doubler_err_uncs[1:])*1e3, yerr=np.array(doubler_std_uncs[1:])*1e3, linestyle='--', marker=marker_doub[0], color=COLOR_DOUB, label="Doubler drive, new measurement system", capsize=CSIZE, alpha=ERROR_ALPHA, markersize=marker_doub[1])
ax2a.errorbar(tri_sigmas, np.array(tri_err_uncs)*1e3, yerr=np.array(tri_std_uncs)*1e3, linestyle='--', marker=marker_tri[0], color=COLOR_TRIPLE, label="Tripler drive, new measurement system", capsize=CSIZE, alpha=ERROR_ALPHA, markersize=marker_tri[1])

ax2a.plot(RP23A_trad_sigma, np.array(RP23A_trad_err)*1e3, linestyle=':', marker='s', color=[0, 0, 0], label='Direct drive, first measurement system')
ax2a.plot(RP23A_trad_sigma, np.array(RP23A_doubler_err)*1e3, linestyle=':', marker='o', color=[0.65, 0, 0], label='Doubler drive, first measurement system')
